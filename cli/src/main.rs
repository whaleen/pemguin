...
enum ProjectTab { Home, Issues, Setup, Prompts, Memories, Skills, Mcp, Pane }

#[derive(PartialEq, Clone)]
enum MemoriesView { Project, Global, Claude }

#[derive(PartialEq, Clone)]
enum PromptsView { Global, Project }

#[derive(PartialEq, Clone)]
enum HomeEditField { Description, Homepage }
...
fn detect_stack(path: &Path) -> Option<String> {
    if let Ok(s) = fs::read_to_string(path.join("Cargo.toml")) {
        let name = s.lines().find_map(|l| l.strip_prefix("name = ").map(|v| v.trim_matches('"').to_string()));
        return Some(format!("Rust ({})", name.as_deref().unwrap_or("?")));
    }
    if let Ok(s) = fs::read_to_string(path.join("package.json")) {
        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&s) {
            let name = v["name"].as_str().unwrap_or("?");
            let stack = if path.join("next.config.ts").exists() || path.join("next.config.js").exists() {
                "Next.js"
...
fn load_home_data(path: &Path, repo: &str) -> HomeData {
    let (gh_description, homepage) = if !repo.is_empty() {
        ...
        if let Some(out) = out {
            if let Ok(v) = serde_json::from_slice::<serde_json::Value>(&out.stdout) {
                let desc = v["description"].as_str().filter(|s| !s.is_empty()).map(|s| s.to_string());
                let home = v["homepageUrl"].as_str().filter(|s| !s.is_empty()).map(|s| s.to_string());
                (desc, home)
            } else { (None, None) }
        } else { (None, None) }
    } else { (None, None) };

    let url = if repo.contains('/') { format!("https://github.com/{repo}") } else { String::new() };
    ...
    HomeData { gh_description, homepage, url, recent_commits, setup_ok, setup_total, stack }
}

// ── Setup screen ──────────────────────────────────────────────────────────────

#[derive(PartialEq, Clone)]
enum SetupStatus { Ok, Missing, Stale }
...
fn scan_setup(path: &Path) -> Vec<SetupItem> {
    ...
    let mut items = vec![
        SetupItem { label: "AGENT.md",          detail: "agent context file",           status: if agent_ok    { SetupStatus::Ok } else { SetupStatus::Missing } },
        SetupItem { label: "SPEC.md",            detail: "feature spec",                 status: if spec_ok     { SetupStatus::Ok } else { SetupStatus::Missing } },
        SetupItem { label: "CLAUDE.md → AGENT.md", detail: "symlink for Claude Code",   status: if claude_ok   { SetupStatus::Ok } else { SetupStatus::Missing } },
        SetupItem { label: "GEMINI.md → AGENT.md", detail: "symlink for Gemini",        status: if gemini_ok   { SetupStatus::Ok } else { SetupStatus::Missing } },
        SetupItem { label: "docs/",              detail: "architecture/features skeleton", status: if docs_ok   { SetupStatus::Ok } else { SetupStatus::Missing } },
        SetupItem { label: ".gitignore",         detail: "agent dirs excluded",          status: if gitignore_ok { SetupStatus::Ok } else { SetupStatus::Missing } },
        SetupItem { label: ".prompts/",          detail: "project-local prompts",        status: if prompts_ok  { SetupStatus::Ok } else { SetupStatus::Missing } },
        SetupItem { label: ".memory/",           detail: "agent memory index",           status: if memory_ok   { SetupStatus::Ok } else { SetupStatus::Missing } },
    ];
    if agents_stale {
        items.push(SetupItem { label: "AGENTS.md", detail: "stale file — delete it", status: SetupStatus::Stale });
    }
    items
}
...
fn apply_setup_item(project_path: &Path, item: &SetupItem) -> Result<String, String> {
    ...
    match item.label {
        "AGENT.md" => {
            fs::copy(pemguin_dir.join("AGENT.md"), project_path.join("AGENT.md"))
                .map(|_| "Created AGENT.md".to_string())
                .map_err(|e| e.to_string())
        }
        "SPEC.md" => {
            fs::copy(pemguin_dir.join("SPEC.md"), project_path.join("SPEC.md"))
        ...
        ".prompts/" => {
            ...
            if !sample.exists() {
                fs::write(&sample, SAMPLE_PROJECT_PROMPT).map_err(|e| e.to_string())?;
            }
            Ok("Created .prompts/ with sample prompt".to_string())
        }
        "AGENTS.md" => {
            fs::remove_file(project_path.join("AGENTS.md"))
...
enum PromptState {
    Browse { list_state: ListState },
    Fill { prompt_idx: usize, field_idx: usize, values: HashMap<String, String>, input: String },
    Done(String),
}
...
fn load_prompts_from(dir: &Path) -> Vec<Prompt> {
    ...
    entries.retain(|e| e.path().extension().and_then(|s| s.to_str()).map(|s| s == "md").unwrap_or(false));
    ...
    entries.iter().filter_map(|entry| {
        ...
        for cap in re.captures_iter(&body) {
            ...
            if !placeholders.contains(&p) { placeholders.push(p); }
        }
        Some(Prompt { name, body, preview: content, placeholders })
    }).collect()
}

fn global_prompts_dir() -> PathBuf {
    dirs_home().unwrap_or_default().join(".pemguin").join("prompts")
}
...
fn extract_body(content: &str) -> String {
    ...
    for line in content.lines() {
        if line.starts_with("```") && !in_block { in_block = true; continue; }
        if line.starts_with("```") && in_block {
            if !block.is_empty() { return block.join("\n"); }
            in_block = false; block.clear(); continue;
        }
        if in_block { block.push(line); }
    }
    content.to_string()
}
...
fn load_issues(repo: &str) -> Result<Vec<Issue>, String> {
    if repo.is_empty() { return Err("No repo context".to_string()); }
    let output = Command::new("gh")
        .args(["issue", "list", "--repo", repo, "--json", "number,title,body,labels,state", "--limit", "50"])
        .output()
...
fn scan_projects(config: &Config) -> Vec<Project> {
    let base = std::env::var("PEMGUIN_PROJECTS_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            config.projects.root.as_ref()
                .map(|r| PathBuf::from(expand_tilde(r)))
                .unwrap_or_else(|| dirs_home().map(|h| h.join("Projects")).unwrap_or_else(|| PathBuf::from(".")))
        });

    let mut projects: Vec<Project> = Vec::new();

    // Walk up to 2 levels for .git dirs
    let Ok(level1) = fs::read_dir(&base) else { return projects; };
    let mut level1_dirs: Vec<_> = level1.filter_map(|e| e.ok()).filter(|e| e.path().is_dir()).collect();
    ...
    for entry in level1_dirs {
        ...
        if path.join(".git").is_dir() {
            if let Some(p) = project_info(&path, "".to_string()) { projects.push(p); }
        } else if let Ok(level2) = fs::read_dir(&path) {
            let mut level2_dirs: Vec<_> = level2.filter_map(|e| e.ok()).filter(|e| e.path().is_dir()).collect();
            level2_dirs.sort_by_key(|e| e.file_name());
            for sub in level2_dirs {
                let sub_path = sub.path();
                if sub_path.join(".git").is_dir() {
                    if let Some(p) = project_info(&sub_path, dir_name.clone()) { projects.push(p); }
                }
            }
        }
    }
...
fn project_info(path: &Path, group: String) -> Option<Project> {
    ...
    let branch = git_in(path, &["branch", "--show-current"]).unwrap_or_else(|| "?".to_string());
    let status_out = git_in(path, &["status", "--porcelain"]).unwrap_or_default();
    let is_dirty = !status_out.is_empty();
    let ahead: u32 = git_in(path, &["rev-list", "--count", "@{u}..HEAD"])
        .and_then(|s| s.parse().ok()).unwrap_or(0);
    let setup_items = scan_setup(path);
    let setup_total = setup_items.len();
    let setup_ok = setup_items.iter().filter(|i| i.status == SetupStatus::Ok).count();
    Some(Project { path: path.to_path_buf(), group, repo, branch, is_dirty, commits_ahead: ahead, setup_ok, setup_total })
}
...
fn build_project_entries(projects: &[Project]) -> Vec<ProjectEntry> {
    ...
    for (i, p) in projects.iter().enumerate() {
        let group_str = if p.group.is_empty() { None } else { Some(p.group.as_str()) };
...
fn meta_cache_path() -> PathBuf {
    dirs_home().unwrap_or_default().join(".pemguin").join("cache.json")
}
...
fn sync_meta(projects: &[Project]) -> HashMap<String, RepoMeta> {
    ...
    let mut orgs: Vec<String> = projects.iter()
    ...
    for org in &orgs {
        ...
        let Ok(out) = Command::new("gh")
            .args(["repo", "list", org, "--limit", "100",
                   "--json", "name,primaryLanguage,repositoryTopics,pushedAt"])
            .output() else { continue };
        if !out.status.success() { continue; }
        let Ok(arr) = serde_json::from_slice::<serde_json::Value>(&out.stdout) else { continue };
        let Some(items) = arr.as_array() else { continue };
        for item in items {
            let name = item["name"].as_str().unwrap_or("").to_string();
            if name.is_empty() { continue; }
            let language = item["primaryLanguage"]["name"].as_str().map(|s| s.to_string());
            let topics = item["repositoryTopics"].as_array().unwrap_or(&vec![])
                .iter().filter_map(|t| t["name"].as_str().map(|s| s.to_string())).collect();
            let pushed_at = item["pushedAt"].as_str().map(|s| s.to_string());
            cache.insert(format!("{org}/{name}"), RepoMeta { language, topics, pushed_at });
        }
    }
    cache
}
...
fn lang_short(lang: &str) -> &str {
    match lang {
        ...
        "Go" => "Go", "Python" => "Py", "Ruby" => "Rb",
        "CSS" => "CS", "HTML" => "HT", "Shell" => "SH", "Svelte" => "SV",
        "Solidity" => "So", "Nix" => "Nx",
...
fn relative_date(iso: &str) -> String {
    ...
    let parts: Vec<u64> = iso.splitn(2, 'T').next().unwrap_or("")
...
fn git_in(dir: &Path, args: &[&str]) -> Option<String> {
    Command::new("git").current_dir(dir).args(args).output().ok()
        .filter(|o| o.status.success())
        .map(|o| String::from_utf8_lossy(&o.stdout).trim().to_string())
        .filter(|s| !s.is_empty())
}
...
fn global_memory_path() -> PathBuf {
    dirs_home().unwrap_or_default().join(".pemguin").join("memory")
}
...
fn claude_memory_path(project_path: &Path) -> PathBuf {
    ...
    dirs_home().unwrap_or_default()
        .join(".claude").join("projects").join(sanitized).join("memory")
}
...
fn load_skills(path: &Path) -> Vec<Skill> {
    ...
    for (name, val) in &skills_obj {
        ...
        let skill_md = path.join(".agents").join("skills").join(name).join("SKILL.md");
        let description = fs::read_to_string(&skill_md)
            ...
            .unwrap_or_default();
        skills.push(Skill { name: name.clone(), source, description });
    }
...
fn load_mcp_servers(path: &Path) -> Vec<McpServer> {
    ...
    for (name, val) in &servers {
        ...
        let args: Vec<String> = val.get("args")
            ...
            .unwrap_or_default();
        result.push(McpServer { name: name.clone(), command, args });
    }
...
fn avatar_dir() -> PathBuf {
    dirs_home().unwrap_or_default().join(".pemguin").join("avatars")
}
...
fn fetch_avatar(owner: &str) -> Option<String> {
    ...
    let out = Command::new("chafa")
        .args(["--size", "20x10", "--format", "symbols", png.to_str().unwrap_or("")])
        .output().ok()?;
    if !out.status.success() { return None; }
...
fn ansi_to_lines(s: &str) -> Vec<Line<'static>> {
    ...
    if !text.is_empty() { spans.push(Span::styled(text, style)); }
...
fn ansi256(n: u8) -> Color {
    match n {
        ...
        232..=255 => { let v = (n - 232) * 10 + 8; Color::Rgb(v, v, v) }
    }
}
...
impl App {
    fn new(config: Config) -> Self {
        ...
        let project_entries = build_project_entries(&projects);
        let global_prompts = load_prompts_from(&global_prompts_dir());

        let mut prompt_ls = ListState::default();
        if !global_prompts.is_empty() { prompt_ls.select(Some(0)); }
        let mut project_ls = ListState::default();
        if let Some(first_item) = project_entries.iter().position(|e| matches!(e, ProjectEntry::Item(_))) {
        ...
        App {
            ...
            memory_files: vec![], memory_list_state: ListState::default(),
            memory_message: None, memory_input: String::new(), memory_inputting: false,
            pending_editor: None,
            skills: vec![],
            skills_list_state: { let mut s = ListState::default(); s.select(Some(0)); s },
            mcp_servers: vec![],
            mcp_list_state: { let mut s = ListState::default(); s.select(Some(0)); s },
            context: String::new(), repo: String::new(),
        }
    }
    ...
    fn switch_prompts_view(&mut self, view: PromptsView) {
        ...
        if !self.prompts.is_empty() { ls.select(Some(0)); }
    ...
    fn auto_values(&self) -> HashMap<String, String> {
        ...
        if !self.repo.is_empty() { map.insert("REPO".to_string(), self.repo.clone()); }
        map
    }

    fn selected_prompt_idx(&self) -> Option<usize> {
        if let PromptState::Browse { list_state } = &self.prompt_state { list_state.selected() } else { None }
    }
    ...
    fn switch_project(&mut self, idx: usize) {
        ...
        self.project_prompts = load_prompts_from(&path.join(".prompts"));
        let view = if !self.project_prompts.is_empty() { PromptsView::Project } else { PromptsView::Global };
        self.switch_prompts_view(view);
        // Load home data
        self.home_data = Some(load_home_data(&path, &self.repo.clone()));
        // Load issues
        match load_issues(&self.repo) {
            Ok(issues) => {
                let mut ls = ListState::default();
                if !issues.is_empty() { ls.select(Some(0)); }
                self.issues = issues;
        ...
        // Load setup
        self.setup_items = scan_setup(&path);
        if !self.setup_items.is_empty() { self.setup_list_state.select(Some(0)); }
        self.setup_message = None;
        // Load memories — default to Claude view if it has content, else Project
        let claude_dir = claude_memory_path(&path);
        let has_claude = claude_dir.is_dir() && fs::read_dir(&claude_dir).map(|mut d| d.next().is_some()).unwrap_or(false);
        self.memories_view = if has_claude { MemoriesView::Claude } else { MemoriesView::Project };
        self.reload_memories();
        self.memory_message = None;
        // Load skills and MCP
        self.skills = load_skills(&path);
        self.skills_list_state = { let mut s = ListState::default(); if !self.skills.is_empty() { s.select(Some(0)); } s };
        self.mcp_servers = load_mcp_servers(&path);
        self.mcp_list_state = { let mut s = ListState::default(); if !self.mcp_servers.is_empty() { s.select(Some(0)); } s };
        // Load avatar (cached per owner)
        let owner = self.repo.split('/').next().unwrap_or("").to_string();
        if !owner.is_empty() && !self.avatar_cache.contains_key(&owner) {
            if let Some(ansi) = fetch_avatar(&owner) {
                self.avatar_cache.insert(owner, ansi);
            }
        }
        // Drill in
    ...
    fn memory_dir(&self) -> PathBuf {
        match self.memories_view {
            MemoriesView::Project => self.active_project_idx
                .and_then(|i| self.projects.get(i))
                .map(|p| p.path.join(".memory"))
                .unwrap_or_default(),
            MemoriesView::Global  => global_memory_path(),
            MemoriesView::Claude  => self.active_project_idx
                .and_then(|i| self.projects.get(i))
    ...
    fn reload_memories(&mut self) {
        ...
        let mut ls = ListState::default();
        if !self.memory_files.is_empty() { ls.select(Some(0)); }
...
fn make_context(repo: &str, branch: &str) -> String {
    if repo.is_empty() { format!("no repo ({branch})") } else { format!("{repo} ({branch})") }
}
...
const SAMPLE_PROJECT_PROMPT: &str = "# Work on this project\n\nEdit this prompt to describe recurring work for this project.\n\n```\nBefore touching any code:\n1. Read AGENT.md\n2. Read SPEC.md\n\n[describe the work here]\n\nWhen done:\n1. Run the project check and build commands — fix any errors\n2. Commit with a clear message\n3. Do not open a PR\n```\n";
...
fn handle_key(app: &mut App, key: KeyCode, modifiers: KeyModifiers) -> bool {
    if key == KeyCode::Char('c') && modifiers.contains(KeyModifiers::CONTROL) { return true; }

    match &app.screen {
        Screen::Projects => handle_projects(app, key),
        Screen::InProject(_) => {
            // Fill/Done, home-edit, and memory-input capture all keys before global nav
            let in_flow = matches!(&app.prompt_state, PromptState::Fill { .. } | PromptState::Done(_))
                || app.home_edit.is_some()
                || app.memory_inputting;
            if !in_flow {
                match key {
                    KeyCode::Esc => { app.screen = Screen::Projects; return false; }
                    KeyCode::Char('q') => return true,
                    KeyCode::Tab => {
                        app.screen = match &app.screen {
                            Screen::InProject(ProjectTab::Home)     => Screen::InProject(ProjectTab::Issues),
                            Screen::InProject(ProjectTab::Issues)   => Screen::InProject(ProjectTab::Setup),
                            Screen::InProject(ProjectTab::Setup)    => Screen::InProject(ProjectTab::Prompts),
                            Screen::InProject(ProjectTab::Prompts)  => Screen::InProject(ProjectTab::Memories),
                            Screen::InProject(ProjectTab::Memories) => Screen::InProject(ProjectTab::Skills),
                            Screen::InProject(ProjectTab::Skills)   => Screen::InProject(ProjectTab::Mcp),
                            Screen::InProject(ProjectTab::Mcp)      => Screen::InProject(ProjectTab::Pane),
                            Screen::InProject(ProjectTab::Pane)     => Screen::InProject(ProjectTab::Home),
                            _ => Screen::InProject(ProjectTab::Home),
                        };
                        return false;
                    }
                    KeyCode::Char('1') => { app.screen = Screen::InProject(ProjectTab::Home);     return false; }
                    KeyCode::Char('2') => { app.screen = Screen::InProject(ProjectTab::Issues);   return false; }
                    KeyCode::Char('3') => { app.screen = Screen::InProject(ProjectTab::Setup);    return false; }
                    KeyCode::Char('4') => { app.screen = Screen::InProject(ProjectTab::Prompts);  return false; }
                    KeyCode::Char('5') => { app.screen = Screen::InProject(ProjectTab::Memories); return false; }
                    KeyCode::Char('6') => { app.screen = Screen::InProject(ProjectTab::Skills); return false; }
                    KeyCode::Char('7') => { app.screen = Screen::InProject(ProjectTab::Mcp);    return false; }
                    KeyCode::Char('8') => { app.screen = Screen::InProject(ProjectTab::Pane);   return false; }
                    _ => {}
                }
            }
            // Dispatch to sub-screen handler
            let tab = if let Screen::InProject(t) = &app.screen { t.clone() } else { return false; };
            match tab {
...
fn handle_prompts(app: &mut App, key: KeyCode) -> bool {
    ...
    if matches!(&app.prompt_state, PromptState::Browse { .. }) {
        match key {
            KeyCode::Char('g') => { app.switch_prompts_view(PromptsView::Global);  return false; }
            KeyCode::Char('p') => { app.switch_prompts_view(PromptsView::Project); return false; }
            _ => {}
        }
    }

    let auto = app.auto_values();
    let fillable_cache: Vec<String> = if let PromptState::Fill { prompt_idx, .. } = &app.prompt_state {
        let idx = *prompt_idx;
        app.prompts[idx].placeholders.iter().filter(|p| !auto.contains_key(*p)).cloned().collect()
    } else { Vec::new() };
    ...
    match &mut app.prompt_state {
        PromptState::Browse { list_state } => {
            ...
            match key {
                KeyCode::Down | KeyCode::Char('j') => { let n = (list_state.selected().unwrap_or(0) + 1) % len; list_state.select(Some(n)); }
                KeyCode::Up   | KeyCode::Char('k') => { let n = list_state.selected().map(|i| if i == 0 { len - 1 } else { i - 1 }).unwrap_or(0); list_state.select(Some(n)); }
                KeyCode::Enter => {
                    if let Some(idx) = list_state.selected() {
                        let prompt = &app.prompts[idx];
                        let fillable: Vec<String> = prompt.placeholders.iter().filter(|p| !auto.contains_key(*p)).cloned().collect();
                        if fillable.is_empty() {
                            let filled = fill(&prompt.body, &auto);
                            copy_to_clipboard(&filled);
                            app.prompt_state = PromptState::Done(filled);
                        } else {
                            app.prompt_state = PromptState::Fill { prompt_idx: idx, field_idx: 0, values: auto.clone(), input: String::new() };
                        }
                    }
                }
                _ => {}
        ...
        PromptState::Fill { prompt_idx, field_idx, values, input } => {
            let fillable = &fillable_cache;
            match key {
                KeyCode::Esc => { let idx = *prompt_idx; let mut ls = ListState::default(); ls.select(Some(idx)); app.prompt_state = PromptState::Browse { list_state: ls }; }
                ...
                KeyCode::Enter => {
                    if *field_idx < fillable.len() {
                        ...
                        *field_idx += 1;
                        if *field_idx >= fillable.len() {
                            let v = values.clone(); let b = app.prompts[*prompt_idx].body.clone();
                            let filled = fill(&b, &v); copy_to_clipboard(&filled);
                            app.prompt_state = PromptState::Done(filled);
                        }
                    }
                }
        ...
        PromptState::Done(_) => { let mut ls = ListState::default(); ls.select(Some(0)); app.prompt_state = PromptState::Browse { list_state: ls }; }
    }
    false
}
...
fn handle_home(app: &mut App, key: KeyCode) -> bool {
    ...
    if let Some(field) = app.home_edit.clone() {
        match key {
            ...
            KeyCode::Backspace => { app.home_edit_input.pop(); }
            KeyCode::Char(c) => { app.home_edit_input.push(c); }
            KeyCode::Enter => {
                ...
                match save_repo_field(&repo, &field, &value) {
                    Ok(_) => {
                        if let Some(data) = &mut app.home_data {
                            match field {
                                HomeEditField::Description => data.gh_description = if value.is_empty() { None } else { Some(value) },
                                HomeEditField::Homepage    => data.homepage = if value.is_empty() { None } else { Some(value) },
                            }
                        }
                        app.home_save_msg = Some("Saved.".to_string());
                    }
                    Err(e) => { app.home_save_msg = Some(format!("Error: {e}")); }
                }
                app.home_edit = None;
                app.home_edit_input.clear();
            }
    ...
    match key {
        KeyCode::Char('r') => {
            if let Some(idx) = app.active_project_idx {
                if let Some(p) = app.projects.get(idx) {
                    let path = p.path.clone();
                    let repo = app.repo.clone();
                    app.home_data = Some(load_home_data(&path, &repo));
                    app.home_save_msg = None;
                }
            }
        }
        KeyCode::Char('e') => {
            let current = app.home_data.as_ref().and_then(|d| d.gh_description.clone()).unwrap_or_default();
            app.home_edit = Some(HomeEditField::Description);
            app.home_edit_input = current;
            app.home_save_msg = None;
        }
        KeyCode::Char('u') => {
            let current = app.home_data.as_ref().and_then(|d| d.homepage.clone()).unwrap_or_default();
        ...
        KeyCode::Char('y') => {
            if let Some(data) = &app.home_data {
                if !data.url.is_empty() { copy_to_clipboard(&data.url); }
            }
        }
        _ => {}
    }
...
fn handle_issues(app: &mut App, key: KeyCode) -> bool {
    ...
    match key {
        KeyCode::Down | KeyCode::Char('j') if len > 0 => { let n = (app.issue_list_state.selected().unwrap_or(0) + 1) % len; app.issue_list_state.select(Some(n)); }
        KeyCode::Up   | KeyCode::Char('k') if len > 0 => { let n = app.issue_list_state.selected().map(|i| if i == 0 { len - 1 } else { i - 1 }).unwrap_or(0); app.issue_list_state.select(Some(n)); }
        KeyCode::Enter => {
        ...
        KeyCode::Char('r') => {
            match load_issues(&app.repo) {
                Ok(issues) => { let mut ls = ListState::default(); if !issues.is_empty() { ls.select(Some(0)); } app.issues = issues; app.issue_list_state = ls; app.issues_error = None; }
                Err(e) => { app.issues_error = Some(e); }
            }
        }
        _ => {}
    }
...
fn next_item_entry(entries: &[ProjectEntry], from: usize, step: isize) -> usize {
    ...
    for _ in 0..len {
        if matches!(entries[i], ProjectEntry::Item(_)) { return i; }
...
fn handle_projects(app: &mut App, key: KeyCode) -> bool {
    ...
    match key {
        ...
        KeyCode::Down | KeyCode::Char('j') if elen > 0 => {
            ...
            app.project_list_state.select(Some(next_item_entry(&app.project_entries, cur, 1)));
        }
...
fn fill(body: &str, values: &HashMap<String, String>) -> String {
    ...
    for (k, v) in values { r = r.replace(&format!("{{{k}}}"), v); }
    r
}
...
fn draw(frame: &mut Frame, app: &App) {
    match (&app.screen, &app.prompt_state) {
        (_, PromptState::Fill { prompt_idx, field_idx, values, input }) => draw_fill(frame, app, *prompt_idx, *field_idx, values, input),
...
fn tab_span(icon: &str, label: &str, n: u8, active: bool) -> Vec<Span<'static>> {
    ...
    if active {
        vec![Span::styled(text, Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD))]
    } else {
...
fn header_row(app: &App) -> Line<'static> {
    let badge = Span::styled(" 🐧 pm ", Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD));
    match &app.screen {
        Screen::Projects => Line::from(vec![
            badge,
            Span::raw("  "),
            Span::styled(format!(" {I_PROJECTS} projects "), Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD)),
        ]),
        Screen::InProject(_) => {
            let repo_short = app.repo.split('/').last().unwrap_or(&app.repo).to_string();
            let branch = app.context.split('(').nth(1).unwrap_or("").trim_end_matches(')').to_string();
            let mut spans = vec![
                badge,
                Span::raw("  "),
                Span::styled(repo_short, Style::default().fg(Color::White).add_modifier(Modifier::BOLD)),
            ];
            if !branch.is_empty() {
                spans.push(Span::styled(format!("  {I_BRANCH} {branch}"), Style::default().fg(FG_DIM)));
            }
            Line::from(spans)
        }
    }
...
fn nav_row(app: &App) -> Line<'static> {
    let Screen::InProject(active_tab) = &app.screen else { return Line::from(""); };
    let repo_short = app.repo.split('/').last().unwrap_or(&app.repo).to_string();
    let mut spans: Vec<Span> = Vec::new();
    let tabs: &[(&str, &str, u8, bool)] = &[
        ("",        repo_short.as_str(), 1u8, *active_tab == ProjectTab::Home),
...
fn draw_home(frame: &mut Frame, app: &App) {
    let area = frame.area();
    let bottom_h = if app.home_edit.is_some() || app.home_save_msg.is_some() { 3u16 } else { 0 };
    let outer = Layout::default().direction(Direction::Vertical)
        .constraints([Constraint::Length(1), Constraint::Length(1), Constraint::Min(0), Constraint::Length(bottom_h), Constraint::Length(1)])
        .split(area);
    ...
    if let Some(data) = &app.home_data {
        ...
        let split = Layout::default().direction(Direction::Horizontal)
            .constraints([Constraint::Length(avatar_w), Constraint::Percentage(55), Constraint::Percentage(45)])
            .split(inner);
        ...
        // Topics
        if let Some(meta) = app.active_project_idx
        ...
        // Setup score
        left.push(Line::from(""));
        let setup_color = if data.setup_ok == data.setup_total { C_GREEN } else { C_YELLOW };
        left.push(Line::from(vec![
            Span::styled("  setup    ", Style::default().fg(FG_DIM)),
            Span::styled(format!("{}/{} configured", data.setup_ok, data.setup_total), Style::default().fg(setup_color)),
        ...
        let mut right: Vec<Line> = vec![
            Line::from(Span::styled(format!("  {I_COMMIT} recent commits"), Style::default().fg(FG_XDIM))),
            Line::from(""),
        ];
        if data.recent_commits.is_empty() {
            right.push(Line::from(Span::styled("  no commits", Style::default().fg(Color::DarkGray))));
        } else {
            for commit in &data.recent_commits {
                let (hash, rest) = commit.split_once(' ').unwrap_or(("", commit));
                right.push(Line::from(vec![
                    Span::styled(format!("  {I_COMMIT} {hash}  "), Style::default().fg(FG_DIM)),
        ...
        frame.render_widget(
            Paragraph::new(Span::styled("  Loading…", Style::default().fg(Color::DarkGray))).block(block),
            outer[2],
        );
    }
    ...
    if let Some(field) = &app.home_edit {
        ...
        frame.render_widget(
            ...
            .block(Block::default().borders(Borders::ALL).title(format!(" edit {label} "))),
            outer[3],
        );
    } else if let Some(msg) = &app.home_save_msg {
        let (icon, color) = if msg.starts_with("Error") { (I_CROSS, C_RED) } else { (I_CHECK, C_GREEN) };
        frame.render_widget(
            Paragraph::new(Span::styled(format!("  {icon}  {msg}"), Style::default().fg(color)))
    ...
    let footer_hints = if app.home_edit.is_some() {
        ...
        footer(&[("r","reload"),("e","edit desc"),("u","edit homepage"),("y","copy url"),("2","issues"),("3","setup"),("esc","back")])
    };
...
fn draw_prompts(frame: &mut Frame, app: &App) {
    let area = frame.area();
    let outer = Layout::default().direction(Direction::Vertical)
        .constraints([Constraint::Length(1), Constraint::Length(1), Constraint::Min(0), Constraint::Length(1)]).split(area);

    frame.render_widget(Paragraph::new(header_row(app)), outer[0]);
    frame.render_widget(Paragraph::new(nav_row(app)), outer[1]);

    // Split left pane into subnav + list
    let left_area = Layout::default().direction(Direction::Horizontal)
        .constraints([Constraint::Percentage(30), Constraint::Percentage(70)]).split(outer[2]);

    let list_split = Layout::default().direction(Direction::Vertical)
        .constraints([Constraint::Length(1), Constraint::Min(0)]).split(left_area[0]);
    ...
    let subnav = Line::from(vec![
        Span::styled(" g global ", if global_active  { Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD) } else { Style::default().fg(FG_DIM) }),
        Span::raw(" "),
        Span::styled(" p project ", if project_active { Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD) } else if has_project { Style::default().fg(FG_DIM) } else { Style::default().fg(FG_XDIM) }),
    ]);
    frame.render_widget(Paragraph::new(subnav), list_split[0]);

    let items: Vec<ListItem> = app.prompts.iter().map(|p| ListItem::new(p.name.clone())).collect();
    ...
    let mut ls = if let PromptState::Browse { list_state } = &app.prompt_state { list_state.clone() } else { ListState::default() };
    frame.render_stateful_widget(
        List::new(items).block(Block::default().borders(Borders::ALL).title(block_title))
            .highlight_style(hl())
            .highlight_symbol("> "),
        list_split[1], &mut ls,
    );

    let preview = app.selected_prompt_idx().and_then(|i| app.prompts.get(i)).map(|p| p.preview.as_str()).unwrap_or("");
    frame.render_widget(
        Paragraph::new(preview).block(Block::default().borders(Borders::ALL).title(" preview ")).wrap(Wrap { trim: false }),
        left_area[1],
    );
    frame.render_widget(Paragraph::new(footer(&[("g","global"),("p","project"),("↑↓/jk","navigate"),("enter","select"),("esc","back"),("tab","switch"),("q","quit")])), outer[3]);
}
...
fn draw_issues(frame: &mut Frame, app: &App) {
    ...
    if let Some(err) = &app.issues_error {
        frame.render_widget(
            Paragraph::new(vec![Line::from(""), Line::from(Span::styled(format!("  {I_CROSS}  {err}"), Style::default().fg(C_RED)))])
                .block(Block::default().borders(Borders::ALL).title(" issues ")),
            outer[2],
        );
    } else if app.issues.is_empty() {
        frame.render_widget(
            Paragraph::new(vec![Line::from(""), Line::from(Span::styled("  No open issues.", Style::default().fg(Color::DarkGray)))])
                .block(Block::default().borders(Borders::ALL).title(" issues ")),
            outer[2],
        );
    } else {
        let main = Layout::default().direction(Direction::Horizontal)
            .constraints([Constraint::Percentage(40), Constraint::Percentage(60)]).split(outer[2]);

        let items: Vec<ListItem> = app.issues.iter().map(|issue| {
            let lbl = if issue.labels.is_empty() { String::new() } else { format!(" [{}]", issue.labels.join(", ")) };
        ...
        frame.render_stateful_widget(
            List::new(items).block(Block::default().borders(Borders::ALL).title(" open issues "))
                .highlight_style(hl())
                .highlight_symbol("> "),
            main[0], &mut ls,
        );

        let preview: Vec<Line> = app.issue_list_state.selected().and_then(|i| app.issues.get(i)).map(|issue| {
            let mut lines = vec![
                Line::from(Span::styled(format!("{I_ISSUES} #{} — {}", issue.number, issue.title), Style::default().fg(Color::White).add_modifier(Modifier::BOLD))),
                Line::from(""),
            ];
            if !issue.labels.is_empty() { lines.push(Line::from(vec![Span::styled("Labels: ", Style::default().fg(Color::DarkGray)), Span::raw(issue.labels.join(", "))])); lines.push(Line::from("")); }
            for l in issue.body.lines() { lines.push(Line::from(l.to_string())); }
            lines
        }).unwrap_or_default();

        frame.render_widget(
            Paragraph::new(preview).block(Block::default().borders(Borders::ALL).title(" body ")).wrap(Wrap { trim: false }),
            main[1],
        );
    }
    frame.render_widget(Paragraph::new(footer(&[("↑↓/jk","navigate"),("enter","copy prompt"),("r","refresh"),("esc","back"),("tab","switch"),("q","quit")])), outer[3]);
}

fn draw_projects(frame: &mut Frame, app: &App) {
    let area = frame.area();
    let outer = Layout::default().direction(Direction::Vertical)
        .constraints([Constraint::Length(1), Constraint::Min(0), Constraint::Length(1)]).split(area);
    ...
    if app.projects.is_empty() {
        frame.render_widget(
            Paragraph::new(vec![
                ...
                Line::from(""),
                Line::from(Span::styled("  Set $PEMGUIN_PROJECTS_DIR or place projects under ~/Projects/", Style::default().fg(Color::DarkGray))),
        ...
        let split = Layout::default().direction(Direction::Vertical)
            .constraints([Constraint::Length(1), Constraint::Min(0)]).split(inner);
        ...
        let max_name = app.projects.iter()
        ...
        let items: Vec<ListItem> = app.project_entries.iter().map(|entry| {
            match entry {
                ProjectEntry::Group(name) => {
                    ListItem::new(Line::from(
                        Span::styled(format!("  {name}"), Style::default().fg(C_PURPLE).add_modifier(Modifier::BOLD))
                    ))
                }
                ProjectEntry::Item(proj_idx) => {
                    ...
                    let lang   = meta.and_then(|m| m.language.as_deref()).map(lang_short).unwrap_or("");
                    let pushed = meta.and_then(|m| m.pushed_at.as_deref()).map(relative_date).unwrap_or_default();
                    ...
                    let status = if p.is_dirty {
                        Span::styled(format!(" {I_WARN} dirty  "), Style::default().fg(C_YELLOW))
                    } else {
                        Span::styled(format!(" {I_CHECK} clean  "), Style::default().fg(C_GREEN))
                    };
                    // Both states must be same cell width: icon(2) + space(1) + 2chars + padding = 5 cells
                    let ahead = if p.commits_ahead > 0 {
                        Span::styled(format!("{I_AHEAD} {:<2} ", p.commits_ahead), Style::default().fg(C_PURPLE))
                    } else {
                        Span::raw("     ")
                    };
                    let cfg_color = if p.setup_ok == p.setup_total { C_GREEN }
                        else if p.setup_ok == 0 { C_RED }
                        else { C_YELLOW };
                    // Marker: both states = 3 cells (NF icon is 2 cells + 1 space, or 3 plain spaces)
                    let marker = if active { format!("{I_BULLET} ") } else { "   ".to_string() };

                    ListItem::new(Line::from(vec![
                        Span::styled(marker,                                        Style::default().fg(ACCENT)),
                        Span::styled(format!("{:<repo_col$}", repo_display),        repo_style),
                        Span::styled(format!(" {:<3} ", lang),                      Style::default().fg(C_PURPLE)),
                        Span::styled(format!(" {I_BRANCH} {:<9}", branch_display),  Style::default().fg(FG_DIM)),
                        status, ahead,
                        Span::styled(format!("{:<4}", format!("{}/{}", p.setup_ok, p.setup_total)), Style::default().fg(cfg_color)),
...
fn handle_memories(app: &mut App, key: KeyCode) -> bool {
    ...
    if app.memory_inputting {
        match key {
            KeyCode::Esc => { app.memory_inputting = false; app.memory_input.clear(); }
            KeyCode::Backspace => { app.memory_input.pop(); }
    ...
    match key {
        KeyCode::Char('p') => { app.switch_memories_view(MemoriesView::Project); }
        KeyCode::Char('g') => { app.switch_memories_view(MemoriesView::Global); }
        KeyCode::Char('c') => { app.switch_memories_view(MemoriesView::Claude); }
        ...
        KeyCode::Up | KeyCode::Char('k') if !app.memory_files.is_empty() => {
            ...
            let n = app.memory_list_state.selected().map(|i| if i == 0 { len - 1 } else { i - 1 }).unwrap_or(0);
            app.memory_list_state.select(Some(n));
        }
            ...
            if let Some(idx) = app.memory_list_state.selected() {
                if let Some(f) = app.memory_files.get(idx) {
                    ...
                    match fs::remove_file(&path) {
                        Ok(_) => { app.memory_message = Some(format!("Deleted {name}.md")); app.reload_memories(); }
                        Err(e) => { app.memory_message = Some(format!("Error: {e}")); }
                    }
                }
            }
        }
                    ...
                    if let Some(proj) = app.active_project_idx.and_then(|i| app.projects.get(i)) {
                        ...
                        match fs::copy(&src, &dst) {
                            Ok(_) => {
                                ...
                                app.memory_message = Some(format!("Migrated {} → .memory/", f.name));
        ...
        KeyCode::Char('r') => { app.reload_memories(); app.memory_message = None; }
        _ => {}
    }
    false
}
...
fn handle_setup(app: &mut App, key: KeyCode) -> bool {
    match key {
        ...
        KeyCode::Up | KeyCode::Char('k') if !app.setup_items.is_empty() => {
            ...
            let n = app.setup_list_state.selected().map(|i| if i == 0 { len - 1 } else { i - 1 }).unwrap_or(0);
            app.setup_list_state.select(Some(n));
        }
        KeyCode::Enter => {
                ...
                if let Some(idx) = app.active_project_idx {
                    if let Some(p) = app.projects.get(idx) {
                        ...
                        if item.status != SetupStatus::Ok {
                            app.setup_message = Some(apply_setup_item(&path, &item)
        ...
        KeyCode::Char('a') => {
            if let Some(idx) = app.active_project_idx {
                if let Some(p) = app.projects.get(idx) {
                    let path = p.path.clone();
                    app.setup_message = Some(apply_all_setup(&path)
                        .unwrap_or_else(|e| format!("Error: {e}")));
                    app.refresh_setup();
                }
            }
        }
        KeyCode::Char('r') => { app.refresh_setup(); }
        _ => {}
    }
    false
}
...
fn draw_setup(frame: &mut Frame, app: &App) {
    ...
    if app.active_project_idx.is_none() {
        frame.render_widget(
            Paragraph::new(vec![
                Line::from(""),
                Line::from(Span::styled("  No project selected.", Style::default().fg(Color::DarkGray))),
                Line::from(""),
                Line::from(Span::styled("  Go to [3 projects] and press enter to set the active project.", Style::default().fg(Color::DarkGray))),
            ]).block(Block::default().borders(Borders::ALL).title(" setup ")),
            outer[2],
        );
    } else if app.setup_items.is_empty() {
        frame.render_widget(
            Paragraph::new(Span::styled("  Scanning…", Style::default().fg(Color::DarkGray)))
                .block(Block::default().borders(Borders::ALL).title(" setup ")),
            outer[2],
        );
    } else {
        let inner = Layout::default().direction(Direction::Vertical)
            .constraints([Constraint::Min(0), Constraint::Length(if app.setup_message.is_some() { 3 } else { 0 })])
            .split(outer[2]);

        let items: Vec<ListItem> = app.setup_items.iter().map(|item| {
        ...
        if let Some(msg) = &app.setup_message {
            frame.render_widget(
                Paragraph::new(Span::styled(format!("  {I_CHECK} {msg}"), Style::default().fg(C_GREEN)))
                    .block(Block::default().borders(Borders::ALL)),
                inner[1],
            );
        }
    ...
    frame.render_widget(Paragraph::new(footer(&[("↑↓/jk","navigate"),("enter","apply item"),("a","apply all"),("r","rescan"),("esc","back"),("tab","switch"),("q","quit")])), outer[3]);
}

fn draw_memories(frame: &mut Frame, app: &App) {
    let area = frame.area();
    let input_h = if app.memory_inputting || app.memory_message.is_some() { 3u16 } else { 0 };
    let outer = Layout::default().direction(Direction::Vertical)
        .constraints([Constraint::Length(1), Constraint::Length(1), Constraint::Min(0), Constraint::Length(input_h), Constraint::Length(1)])
        .split(area);
    ...
    let main = Layout::default().direction(Direction::Horizontal)
        .constraints([Constraint::Percentage(35), Constraint::Percentage(65)]).split(outer[2]);

    // Left: subnav + file list
    let left_split = Layout::default().direction(Direction::Vertical)
        .constraints([Constraint::Length(1), Constraint::Min(0)]).split(main[0]);
    ...
    let subnav = Line::from(vec![
        Span::styled(" p project ", if p_active { Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD) } else { Style::default().fg(FG_DIM) }),
        Span::raw(" "),
        Span::styled(" g global ", if g_active { Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD) } else { Style::default().fg(FG_DIM) }),
        Span::raw(" "),
        Span::styled(" c claude ", if c_active { Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD) } else { Style::default().fg(FG_DIM) }),
    ]);
    ...
    let items: Vec<ListItem> = if app.memory_files.is_empty() {
        vec![ListItem::new(Span::styled("  (empty)", Style::default().fg(FG_XDIM)))]
    } else {
        app.memory_files.iter().map(|f| ListItem::new(f.name.clone())).collect()
    };
    let mut ls = app.memory_list_state.clone();
    frame.render_stateful_widget(
        List::new(items)
            .block(Block::default().borders(Borders::ALL).title(dir_label))
            .highlight_style(hl()).highlight_symbol("> "),
        left_split[1], &mut ls,
    );

    // Right: preview
    let preview = app.memory_list_state.selected()
    ...
    if app.memory_inputting {
        frame.render_widget(
            ...
            .block(Block::default().borders(Borders::ALL).title(" new memory name ")),
            outer[3],
        );
    ...
    let footer_hints = if app.memory_inputting {
        ...
        footer(&[("↑↓/jk","navigate"),("e/enter","edit"),("m","migrate → .memory/"),("n","new"),("d","delete"),("r","reload"),("esc","back"),("p/g/c","view")])
    } else {
        footer(&[("↑↓/jk","navigate"),("e/enter","edit"),("n","new"),("d","delete"),("r","reload"),("esc","back"),("p/g/c","view"),("tab","switch")])
    };
    frame.render_widget(Paragraph::new(footer_hints), outer[4]);
}
...
fn draw_skills(frame: &mut Frame, app: &App) {
    ...
    let items: Vec<ListItem> = app.skills.iter().map(|s| ListItem::new(s.name.clone())).collect();

    if items.is_empty() {
        frame.render_widget(
            Paragraph::new(vec![
                Line::from(""),
                Line::from(Span::styled("  no skills installed", Style::default().fg(Color::DarkGray))),
                Line::from(""),
                Line::from(Span::styled("  npx skills add <owner/repo> --skill <name> -y", Style::default().fg(FG_XDIM))),
        ...
        let preview = app.skills_list_state.selected()
            .and_then(|i| app.skills.get(i))
            .map(|s| {
                let mut lines = vec![
                    Line::from(Span::styled(s.name.clone(), Style::default().fg(Color::White).add_modifier(Modifier::BOLD))),
                    Line::from(""),
                    Line::from(vec![Span::styled("source  ", Style::default().fg(FG_DIM)), Span::raw(s.source.clone())]),
                ];
    ...
    frame.render_widget(Paragraph::new(footer(&[("↑↓/jk","navigate"),("esc","back"),("tab","switch"),("q","quit")])), outer[3]);
}
...
fn draw_mcp(frame: &mut Frame, app: &App) {
    ...
    let items: Vec<ListItem> = app.mcp_servers.iter().map(|s| ListItem::new(s.name.clone())).collect();

    if items.is_empty() {
        frame.render_widget(
            Paragraph::new(vec![
                Line::from(""),
                Line::from(Span::styled("  no .mcp.json found", Style::default().fg(Color::DarkGray))),
        ...
        let preview = app.mcp_list_state.selected()
            ...
            .map(|s| {
                ...
                vec![
                    Line::from(Span::styled(s.name.clone(), Style::default().fg(Color::White).add_modifier(Modifier::BOLD))),
                    Line::from(""),
                    Line::from(vec![Span::styled("command  ", Style::default().fg(FG_DIM)), Span::raw(cmd)]),
                ]
            })
            .unwrap_or_default();
...
fn draw_pane(frame: &mut Frame, app: &App) {
    ...
    frame.render_widget(Paragraph::new(footer(&[("esc","back"),("tab","switch"),("q","quit")])), outer[3]);
}
...
fn handle_skills(app: &mut App, key: KeyCode) -> bool {
    ...
    match key {
        ...
        KeyCode::Up | KeyCode::Char('k') => {
            let n = app.skills_list_state.selected().map(|i| if i == 0 { len - 1 } else { i - 1 }).unwrap_or(0);
            app.skills_list_state.select(Some(n));
        }
        _ => {}
    }
...
fn handle_mcp(app: &mut App, key: KeyCode) -> bool {
    ...
    match key {
        ...
        KeyCode::Up | KeyCode::Char('k') => {
            let n = app.mcp_list_state.selected().map(|i| if i == 0 { len - 1 } else { i - 1 }).unwrap_or(0);
            app.mcp_list_state.select(Some(n));
        }
        _ => {}
    }
...
fn draw_fill(frame: &mut Frame, app: &App, prompt_idx: usize, field_idx: usize, values: &HashMap<String, String>, input: &str) {
    let area = frame.area();
    let outer = Layout::default().direction(Direction::Vertical)
        .constraints([Constraint::Length(1), Constraint::Min(0), Constraint::Length(1)]).split(area);
    ...
    let auto = app.auto_values();
    let fillable: Vec<&String> = prompt.placeholders.iter().filter(|p| !auto.contains_key(*p)).collect();

    frame.render_widget(Paragraph::new(Line::from(vec![
        Span::styled(" 🐧 pm ", Style::default().fg(SEL_FG).bg(ACCENT).add_modifier(Modifier::BOLD)),
        Span::raw("  "), Span::styled(&prompt.name, Style::default().fg(Color::White)),
    ])), outer[0]);

    let mut lines: Vec<Line> = vec![Line::from(""), Line::from(Span::styled("  Fill in placeholders", Style::default().fg(Color::White).add_modifier(Modifier::BOLD))), Line::from("")];
    for placeholder in &prompt.placeholders {
        if let Some(v) = auto.get(placeholder) {
            lines.push(Line::from(vec![Span::raw(format!("  {placeholder:<14}")), Span::styled(v.clone(), Style::default().fg(FG_DIM)), Span::styled("  (auto)", Style::default().fg(FG_XDIM))]));
            continue;
        }
        let fi = fillable.iter().position(|p| *p == placeholder).unwrap_or(0);
        if fi < field_idx {
            let val = values.get(placeholder).map(|s| s.as_str()).unwrap_or("");
            lines.push(Line::from(vec![Span::raw(format!("  {placeholder:<14}")), Span::styled(val.to_string(), Style::default().fg(C_GREEN)), Span::styled(format!("  {I_CHECK}"), Style::default().fg(C_GREEN))]));
        } else if fi == field_idx {
            lines.push(Line::from(vec![Span::styled(format!("  {placeholder:<14}"), Style::default().fg(ACCENT).add_modifier(Modifier::BOLD)), Span::styled(format!("{input}█"), Style::default().fg(Color::White))]));
        } else {
            lines.push(Line::from(vec![Span::raw(format!("  {placeholder:<14}")), Span::styled("...", Style::default().fg(FG_XDIM))]));
        }
    }
    frame.render_widget(Paragraph::new(lines).block(Block::default().borders(Borders::ALL)), outer[1]);
    frame.render_widget(Paragraph::new(footer(&[("enter","confirm"),("esc","back")])), outer[2]);
}
...
fn draw_done(frame: &mut Frame, text: &str) {
    ...
    let mut lines: Vec<Line> = vec![
        Line::from(""),
        Line::from(vec![Span::styled(format!("  {I_CHECK}  "), Style::default().fg(C_GREEN)), Span::styled("Copied to clipboard", Style::default().fg(Color::White).add_modifier(Modifier::BOLD))]),
        Line::from(""),
        Line::from(Span::styled("  ─────────────────────────────", Style::default().fg(FG_XDIM))),
        Line::from(""),
    ];
    for line in text.lines().take(20) { lines.push(Line::from(Span::styled(format!("  {line}"), Style::default().fg(Color::DarkGray)))); }
    lines.push(Line::from(""));
    lines.push(Line::from(Span::styled("  any key to continue", Style::default().fg(Color::DarkGray))));
    frame.render_widget(Paragraph::new(lines).block(Block::default().borders(Borders::ALL).title(" pemguin ")), area);
}
...
fn run(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, app: &mut App) -> io::Result<()> {
    loop {
        ...
        if let Event::Key(key) = event::read()? {
            if handle_key(app, key.code, key.modifiers) { break; }
        }
    }
    Ok(())
}
