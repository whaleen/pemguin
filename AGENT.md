...
```
cli/
  src/lib.rs        — entire application (~7800 lines, single file)
  src/main.rs       — thin entry point: calls pemguin::start()
  src/bin/pm.rs     — `pm` binary entry point
...
Colors are loaded at startup from `~/.pemguin.toml` (`[theme]` section) and hot-reloaded on file change (~50ms detection). The theme file is generated by the whaleen dotfiles `theme/generate.sh`. Use `thread_local! { Cell<Option<Theme>> }` — not `RwLock` — for the global theme store; `RwLock` guard deref did not reflect writes in this single-threaded context.

## Gotchas

- **Single source file**: all TUI state, rendering, key handling, and data loading live in `cli/src/lib.rs`.
